        ...
        try {
            if ($isPartner == 1) {
                ...
                if ($this->helper->getIsSeparatePanel()) {
                    $resultPage->addHandle('mpzipcodevalidator_layout2_region_add');
                }
                $resultPage->getConfig()->getTitle()->set(__('Marketplace Add Regions'));
                return $resultPage;
            } else {
            ...
            $this->zipHelper->logDataInLogger(
                "Controller_Region_Add execute : ".$e->getMessage()
            );
        }
    }
}
